...
import time
import random
#Loading Screen (Bounce Ball Game)
root = Tk()
...
score = Label(height=50, width=80, text="Score: 00", font="Calibri 20 italic")
...
#Detects Collison
                try:
                    ...
                    continue
        return False
# Sees if the ball hits the pole
    def pole_strike(self, push):
        ...
        if push[2] >= pole_push[0] and push[0] <= pole_push[2]:
            if push[3] >= pole_push[1] and push[1] <= pole_push[3]:
                ...
                return True
            return False
#Ensures the ball interacts with the stones and pole
    def draw(self):
        self.cvs.move(self.id, self.a, self.b)
...
#Control the movent of the poles
class Pole:
...
#Handles ball movement and regulates
    def draw(self):
        push = self.cvs.coords(self.id)
        # print(pos)
        if push[0] + self.a <= 0:
...
#Stone colors,position, and pole color
def start_game(event):
    ...
    if playing is False:
        ...
        canvas.delete("all")
        BALL_COLOR = ["purple", "grey", "violet", "white"]
        STONE_COLOR = ["dark blue", "dark blue", "dark blue", "dark blue", "violet", "yellow",
                       "Violet", "violet", "violet", "dark blue", "dark blue", "violet",
                       "violet", "dark blue", "dark blue", "powder blue", "violet"]
        ...
        root.update()
#Manages the games state ( Movement, win,loss, and Pause)
        time.sleep(1)
        while 1:
            if pole.pauseSeconds != 1:
                ...
                if not ball.bottom_hit:
                    ...
                    time.sleep(0.01)
                    if ball.hit == 95:
                        canvas.create_text(250, 250, text="C0ngrats you won !", fill="yellow", font="Calibri 24 ")
                        root.update_idletasks()
                        root.update()
                        playing = False
                        break
                else:
                    canvas.create_text(250, 250, text="Better Luck Next time", fill="red", font="Calibri 24 ")
                    root.update_idletasks()
                    root.update()
                    playing = False
                    break
                ...
                try:
                    ...
                    m = canvas.create_text(250, 250, text="PAUSE!!", fill="blue", font="Calibri 24 ")
                root.update_idletasks()
                root.update()

#Starts the game, 
root.bind_all("<Return>", start_game)
canvas.create_text(250, 250, text="Press Enter to start Game!!", fill="blue", font="Calibri 24")
j = canvas.find_all()
root.mainloop()
